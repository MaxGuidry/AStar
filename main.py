...
def drawparents(screen, parents, currentgraph):
    """Draw lines from node to parent."""
    for anode in parents:
        if anode.parent is not None:
            pygame.draw.line(screen, (255, 255, 255),
                             (((screen.get_width() / currentgraph.width) *
                               anode.posx) + (screen.get_width() /
                                              currentgraph.width) / 2,
                              ((screen.get_height() / currentgraph.height) *
                               anode.posy) + (screen.get_height() /
                                              currentgraph.height) / 2),
                             ((screen.get_width() / currentgraph.width) *
                              anode.parent.posx + (screen.get_width() /
                                                   currentgraph.width) / 2,
                              ((screen.get_height() /
                                currentgraph.height) * anode.parent.posy) +
                              (screen.get_width() / currentgraph.width) / 2),
                             2)
...
def drawselected(screen, selectednode, currentgraph):
    ...
    offsety = decimal.Decimal(decimal.Decimal(screen.get_height() %
                                              currentgraph.height) /
                              currentgraph.height) * selectednode.posy
    pygame.draw.rect(screen, (255, 255, 255), pygame.Rect((screen.get_width() /
                                                           currentgraph.width)
                                                          * selectednode.posx +
                                                          offsetx,
                                                          (screen.get_height()
...
def drawstart(screen, startnode, currentgraph):
    ...
    offsety = decimal.Decimal(decimal.Decimal(screen.get_height() %
                                              currentgraph.height) /
                              currentgraph.height) * startnode.posy
    pygame.draw.rect(screen, (5, 230, 35),
                     pygame.Rect((screen.get_width() / currentgraph.width) *
                                 startnode.posx + offsetx,
                                 (screen.get_height() / currentgraph.height)
                                 * startnode.posy + offsety,
                                 screen.get_width() / currentgraph.width,
                                 screen.get_height() / currentgraph.height), 5)
...
def drawwalls(screen, walls, currentgraph):
    ...
    for wall in walls:
        offsetx = decimal.Decimal(decimal.Decimal(screen.get_width() %
                                                  currentgraph.width) /
                                  currentgraph.width) * wall.posx
        offsety = decimal.Decimal(decimal.Decimal(screen.get_height() %
                                                  currentgraph.height) /
                                  currentgraph.height) * wall.posy
        pygame.draw.rect(screen, (200, 50, 180),
                         pygame.Rect((screen.get_width() / currentgraph.width)
                                     * wall.posx + offsetx,
                                     (screen.get_height() /
                                      currentgraph.height) *
